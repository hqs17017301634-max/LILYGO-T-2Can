...
public:
    ...
    // RXM = 0b11: turn masks/filters off and accept any valid frame. Used by the
    // secondary (monitor) bus on dual-CAN boards where no filtering is wanted.
    void setReceiveAllMode()
    {
        bitModify(MCP_RXB0CTRL, RXBnCTRL_RXM_MASK, RXBnCTRL_RXM_MASK);
